<!-- /src/components/CarouselDesktop.vue -->
<template>
    ...
    <transition name="fade-side" mode="out-in">
      ...
      >
        <transition-group name="fade-side" tag="div">
          ...
          >
            <div class="tiny-box" :class="`variant-${(activeIndex * 3 + j) % 4}`">
              {{ detail }}
            </div>
          </div>
        </transition-group>
  ...
  import { sectionIcons, type SectionData } from '../data/sections'
  ...
  const angle = 360 / props.sections.length
  const spacingFactor = 0.7
  
  function getItemStyle(i: number) {
    const spacing = spacingFactor * props.viewportHeight
    const totalScroll = spacing * (props.sections.length - 1)
    const scrollFactor = props.scrollY / totalScroll
    const rotateY = -angle * i + scrollFactor * (angle * (props.sections.length - 1))
    const translateY = i * spacing - props.scrollY + (props.viewportHeight / 2 - props.cardHeight / 2 + 80)
    const focusDist = Math.abs(props.scrollY / spacing - i)
  ...
  function getSideTop(index: number) {
    const spacing = spacingFactor * props.viewportHeight
    const translateY = index * spacing - props.scrollY + (props.viewportHeight / 2 - props.cardHeight / 2)
    const minTop = 20
    const maxTop = props.viewportHeight - props.cardHeight - 20
    return Math.max(minTop, Math.min(translateY, maxTop))
  }
  
  function getTinyBoxStyle(sectionIndex: number, detailIndex: number) {
    const spacing = spacingFactor * props.viewportHeight
    const baseTop = sectionIndex * spacing - props.scrollY + (props.viewportHeight / 2 - props.cardHeight / 2)
    const perDetailOffset = 70
    const top = baseTop + detailIndex * perDetailOffset
    const randomOffset = ((sectionIndex * 13 + detailIndex * 7) % 20) - 30
    const sideShift = ((sectionIndex + detailIndex) % 2 === 0)
      ? `calc(50% - ${360 + randomOffset}px - 180px)`
      : `calc(50% + ${320 + randomOffset}px - 50px)`
    const minTop = 20
    const maxTop = props.viewportHeight - 60
    const clampedTop = Math.max(minTop, Math.min(top + randomOffset, maxTop))
    const variantIndex = (sectionIndex * 3 + detailIndex) % 4
    return {
      position: 'fixed',
      top: `${clampedTop}px`,
      left: sideShift,
